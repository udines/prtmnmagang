...
public class NewRealisasiAdapter extends RecyclerView.Adapter<NewRealisasiViewHolder> {
    ...
    public void onBindViewHolder(@NonNull NewRealisasiViewHolder holder, int position) {
        ...
        holder.realisasi.setText(String.valueOf("Realisasi Rp" + MethodCollection.numberWithDot(anggarans.get(position).getActual())));
        holder.persentase.setText(String.valueOf(
                 MethodCollection.numberWithComma((anggarans.get(position).getActual() / anggarans.get(position).getPlan() * 100)) + "%"
        ));
    }
